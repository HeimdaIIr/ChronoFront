...
class RaspberryController2 extends Controller
{
    /**
     ...
     * ]
     *
     * VERSION 2: Accepts all detections even if reader not configured
     * Allows /rfidlive to display raw detections for testing
     */
    public function store(Request $request): JsonResponse
    {
        ...
        // Get reader configuration (optional for raw display)
        $reader = Reader::getActiveConfig($readerSerial);

        if (!$reader) {
            Log::warning('Reader not configured, detection logged but not processed', [
                'serial' => $readerSerial,
            ]);
